...
export const validateImgUrl = (url, res) => {
  if (!url)
    return res.status(400).json({
      code: 400,
      error: 'The `img` query param with the url is missing.'
    })
}
...
export const validateOrigins = (url, res) => {
  const allowedOrigins =
    ALLOWED_ORIGINS &&
    ALLOWED_ORIGINS.split(',').map((i) => i.replace(/\s/g, ''))
  ...
  if (!originAllowed) {
    ...
    if (!originAllowed)
      return res
        .status(400)
        .json({ code: 400, error: 'Origin url not allowed.' })
  }
}
 ...
 */
export const controlCacheHeader =
  CONTROL_CACHE_HEADER ||
  'max-age=2592000, s-maxage=21600, stale-while-revalidate=86400, public'
